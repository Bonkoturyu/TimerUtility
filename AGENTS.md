...
このファイルは Codex が本プロジェクトで作業する際に **毎セッション最初に読む** 絶対制約集です。
詳細は `docs/` 配下に分離しています。本ファイルは最小限の制約とインデックスのみを記載します。

> **ミラー宣言**: 正典は `CLAUDE.md`。このファイルは直接編集せず、
> CLAUDE.md の変更をツール名置換 (Claude Code→Codex 等) で同期する。
...
- 依頼範囲外の追加改善は実装せず、候補として最終報告に分離する

### 調査・実装・報告の範囲

- 既存コード全体を無目的に再読せず、依頼に必要なファイルと一次資料だけを調査する
- 実装範囲を依頼内容に限定し、追加改善は実装せず候補として報告する
- 長い説明やコード全文の再掲を避け、差分・判断・検証結果を簡潔に報告する
- 同じ方法で 2 回失敗した場合は続行せず、原因候補とユーザー判断が必要な点を報告する
...
4. **AI レビュアー (Copilot / Gemini / Codex 自身を含む) の指摘** — 仮説扱い、
...
Codex 自身の発話でも、外部仕様に関する主張は必ず根拠 URL or
...
- `.github/workflows/`
- `AGENTS.md` および `docs/` 配下のすべてのドキュメント

---

## Auto 運用ポリシー

本プロジェクトは Codex の Auto モードで Phase 別に開発を進める前提。
Auto 起動中の Codex は以下に厳格に従うこと。
