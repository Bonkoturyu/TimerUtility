...
### ClockEntryCollectionNotifier（Phase 10.5 で実装済み、Phase 11 で ClockEntry にリネーム）

```
責務:
- アプリ起動時の DB からの全 ClockEntry 読み込み
- 初回起動時 (DB が空) に LocationDetector.detectTimezoneId() を呼び
  「現在地」エントリを 1 件追加 (isCurrentLocation: true)
- ClockEntry の追加 / 削除 / 並べ替え (上限 6 件、ClockEntryCollection 集約に委譲)
- 変更時に ClockEntryRepository 経由で永続化
...
[Notifier 層]
   ...
   clockEntryCollectionNotifierProvider                         // Phase 10.5 で実装済み、Phase 11 で ClockEntry にリネーム
       │
       ├─ ref.watch(clockEntryRepositoryProvider)
...
| `ClockEntryCollectionNotifier` | (監視なし) | 時刻表示は currentTimeStreamProvider が autoDispose で画面表示中のみ active のため、Notifier 側で lifecycle 監視は不要（Phase 10.5 で実装済み、Phase 11 で ClockEntry にリネーム） |
...
├── clock_entry_collection_notifier.g.dart
