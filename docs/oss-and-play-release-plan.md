...
Play Console 実画面での Store listing / Data Safety 確定のみ進行中、Phase 11.10 は未着手。
...
| T17 | upload keystore 実体生成 + `android/key.properties` 作成後、`flutter build appbundle --release` ローカル成功、AAB サイズ計測 | — | — |
...
- [x] T7: Pixel 6a で launcher icon、Themed Icon (monochrome)、splash cold /
...
- [x] T11: 512 px icon と 1024x500 Feature Graphic は PR #91 で完了。
  ...
  含める方針に更新し、英語版 Feature Graphic
  `design/store/feature-graphic-1024x500-en.png` を追加済み。en スクリーンショットも
  Pixel 6a を英語表示に切り替えて同 7 シナリオを撮影し、
  `design/screenshots/phone/en/` に配置済み。
- [x] T12: release signing 手順書作成済み
- [x] T13: `android/key.properties.template` 追加済み。`android/key.properties`
  実体は gitignore 除外対象として維持
...
- [x] T17: upload keystore 実体生成 + `android/key.properties` 作成後、
...
- Phase 11.9 現時点: 新 applicationId + 新アイコンの aab がローカル build 成功、679 テスト緑、Privacy Policy 公開。残りは Play Console 実画面での Store listing / Data Safety 確定
