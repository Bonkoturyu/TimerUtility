...
| T8.5 | **GitHub Privacy team にメール直送で個人情報削除申請** — 2026-05-16 に `docs/opus-startup-prompt.md` を filter-branch で全 history から削除 + force push 済だが、merged PR #57 が orphan commit `f2e46e3` を参照したままで GitHub cache から旧版 (個人プロファイル含む) が `gh api repos/.../contents/docs/opus-startup-prompt.md?ref=f2e46e3` で **取得可能なまま**。force push だけでは GC が走らないため Support 介入必須。本ケースは「本人が自分のリポジトリから自身の個人情報を削除する」フローのため [Private Information Removal Policy](https://docs.github.com/en/site-policy/content-removal-policies/github-private-information-removal-policy) が指す [Privacy contact form](https://github.com/contact/privacy) を使用。フォームで `Other` を選択すると `privacy@github.com` へのメール直送案内に分岐するため、GitHub 登録メール (`bonkoturyu@gmail.com`) から直接送信。本文には「アカウント削除ではなくリポジトリ単位の purge」と明記し、(i) 影響 PR の dereference / 削除、(ii) サーバー側 GC 実行、(iii) cache 削除 を依頼。**Public 化 (T10) 前に完了必須**。送信内容: リポジトリ名 `Bonkoturyu/TimerUtility` (Private) / 影響 PR 1 件 (#57) / First Changed Commit `f2e46e3` / 対象ファイル `docs/opus-startup-prompt.md` / LFS なし | メール (<privacy@github.com>) | **必須** (不可逆) |
