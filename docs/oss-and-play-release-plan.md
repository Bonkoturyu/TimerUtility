...
| T8.5 | ~~**GitHub Privacy team にメール直送で個人情報削除申請**~~ → **2026-05-27 omit 決定**。2026-05-16 に `bonkoturyu@gmail.com` から `privacy@github.com` 宛にメール直送済みだが、11 日経過 (5/16 → 5/27) しても **auto-ack / ticket 番号 / bounce 通知すべてゼロ** で処理されている形跡なし。並行して orphan commit `f2e46e3` の実物 (`gh api .../contents/docs/opus-startup-prompt.md?ref=f2e46e3`) を取得 → 残留内容を確認したところ、典型 PII (氏名 / 連絡先 / 住所 / financial / credentials / API キー / 写真) が **一切含まれず**、技術スキル列挙 + 自宅 PC 構成 + 使用 SaaS 列挙の粒度のみ (公開 GitHub プロフィール程度の独自性)。ユーザー判断でリスク許容、**本タスクは omit して T10 を T8.6 非依存に変更**。詳細は [docs/dev-log.md](dev-log.md) の「Phase 11.8 T8.5/T8.6 omit 決定 (2026-05-27)」と [memory feedback_filter_branch_github_cache.md](../../.claude/projects/c--AllUsersData-Github-TimerUtility/memory/feedback_filter_branch_github_cache.md) 「コスト・ベネフィット例外」セクション参照 | (omit) | (omit) |
...
| T10 | GitHub Settings → Visibility を Public に変更 + Description / Topics 設定。**T8.5 / T8.6 を omit したため、T9 完了 + Phase 11.8 PR (T1-T9) main マージ済が唯一の前提** (旧記述: 「T8.6 の確認が取れていることが前提」は 2026-05-27 撤回) | GitHub Web | **必須** (不可逆) |
