...
- アプリ起動時の in-memory 状態復元:
  ...
  - Alarm: `AlarmCollectionNotifier._loadFromRepository` (Phase 9.5 +
    ...
    が次回曜日に自動で進むので reschedule のみ。

検証チェックリスト:

- [ ] 端末再起動後にタイマーが復元される
- [ ] 端末再起動後に enabled な alarm が再予約される (パッケージ側 + アプリ起動時 reschedule の二段)
- [ ] 再起動中に過ぎた once-mode alarm が enabled=false に落ちて show 通知 1 回
- [ ] 再起動中に過ぎた weekly alarm が次回曜日に進む
- [ ] 再起動中に過ぎた timer が completed 扱い + show 通知
...
最終更新日: 2026-05-04（Phase 10 起動時復元の戦略 + 検証チェックリストを追記。純 Flutter 採用、Native BootReceiver は新設しない）
