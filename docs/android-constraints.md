...
- [x] 端末再起動後に enabled な alarm が再予約される (manifest exported="true" 修正後の 2 巡目で OK、2026-05-09)
- [x] 再起動中に過ぎた once-mode alarm が enabled=false に落ちて show 通知 1 回 (Force stop + 日時操作 + launcher 起動の 2 巡目手順で OK、2026-05-09)
...
最終更新日: 2026-05-09（Phase 10 完了。Pixel 6a / Android 16 で 4 シナリオ実機検証済、`ScheduledNotificationBootReceiver` の Android 12+ exported 要件不備を確定 + 修正、past-due 検知との二段構えで再起動跨ぎと過去到達 once の両方を救済）
