...
│                   Phase 11 で ClockEntry にリネーム）│
│  ・ClockEntry                                   │
│  ・ClockEntryCollection (集約ルート相当)        │
...
## Clock Aggregate（Phase 10.5 で実装済み、Phase 11 で ClockEntry にリネーム）

「世界時計」機能のドメイン。1 画面で最大 6 都市の現在時刻を見渡す用途。
タイマー / アラームとは独立したタブとして提供される。

Phase 11 のドメインリネーム前は `ClockLocation` / `ClockCollection` /
`ClockLocationRepository` という命名で、presentation 層
(`ClockEntryEditScreen` 系) と概念的に整合していなかった。Drift テーブル
名も含めて `ClockEntry` / `ClockEntryCollection` / `clock_entries` に
揃え、`isCurrentLocation` フィールドだけ GPS 由来の概念として valid な
ため据置している。
...
- `isCurrentLocation = true` のエントリは ClockEntryCollection 全体で 0 件 or 1 件

### ClockEntryCollection（集約ルート）

複数の世界時計エントリを管理する集約。

責務:

- ClockEntry の追加 / 削除 / 並べ替え
- 上限制約の検証
- 現在地エントリの一意性保証
...
- 上限超過時は `MaxClockEntryCountExceededException` を throw
...
ClockEntryCollection {
  ...
  get(ClockEntryId): ClockEntry?
  count(): int
  currentEntry(): ClockEntry?  // isCurrentLocation = true のものを返す
}
```
...
- `ClockScreen` (`lib/presentation/screens/clock_screen.dart`):
  `clockEntryCollectionNotifierProvider.select((c) => c.all)` と
  `currentTimeStreamProvider` を watch、3 デザイン widget
  (`ClockDesignA/B/C`) に `entries` / `now` を props 注入する。
- `ClockEntryEditScreen` (`lib/presentation/screens/clock_entry_edit_screen.dart`):
  `TimezoneCatalog.presets` から都市を add、`ClockEntryCollectionNotifier`
  経由で `ClockEntryCollection` 集約に対する mutation
...
extension type ClockEntryId(String value) {
  factory ClockEntryId.generate() => ClockEntryId(Uuid().v4());
}
