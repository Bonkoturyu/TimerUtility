...
## ユーザー取り込み音源仕様（Phase 13 で実装済み）

本節は、ユーザーが端末ストレージから任意の音源を選び、タイマー / アラーム /
プリセットで利用する実装済み機能の仕様を記録する。現在の配信形態は
「無料・広告なし・アプリ内課金なし」。買い切り枠は将来の利用権限拡張に備えた
設計上の予約であり、現行アプリで販売・提供するものではない。
...
登録不可とする。WAV 等の非圧縮形式では、時間上限に達する前に容量上限へ達し、
実際に登録できる再生時間が短くなる場合がある。
...
- 取り込み開始前の空き容量が 10 GB 未満なら追加不可とする。
- 取り込み後に空き容量が 10 GB 未満になると予測される場合も追加不可とする。
- コピー中に申告サイズを超過した場合は中断し、一時ファイルを登録しない。
- 容量取得に失敗した場合は安全側に倒し、追加せずエラーを表示する。
- 置換時は、旧音源と新音源を一時的に同時保持できる空き容量がある場合だけ実行する。
  件数と合計容量の上限判定では置換対象の旧音源を除外し、新音源の検証・一時コピー・
  DB 更新が成功するまで旧音源を保持する。確定に失敗した場合は旧音源を維持する。
...
音源解決のいずれかに失敗した場合は同梱デフォルト音を使用する。解決処理には期限を
設け、解決遅延によって固定 3200 ms のハンドオフ境界を延長しない。期限内に解決できない
場合は、同梱デフォルト音を境界までに prepare して鳴動する。具体的な内部 timeout 値は、
実装時の自動テストと実機計測で決定する。
...
最終更新日: 2026-08-03（取込音源の実装済み状態と Native Exact 再生へ同期）
