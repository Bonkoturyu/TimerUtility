...
**Chinese の country → script 推定 (TW/HK → Hant) は実装されていない**。
...
`zh_Hant_TW` / `zh_Hant_HK` を明示追加し、language+country 照合で繁体字へ
倒すようにした。生成側の `lookupAppLocalizations` は `scriptCode == 'Hant'`
だけを見るので、どちらの country 変種も `AppLocalizationsZhHant` を読む。
...
zh_Hant_MO / zh_Hans_CN / ko_KR の解決先を追加。
...
**検証**: `flutter analyze` 0 issues、`flutter test` 707 passed / 1 skipped。
...
仕様と設計判断を確定した。現行アプリの配信形態は無料・広告なし・アプリ内課金なしを
維持し、買い切り枠は将来拡張として予約する。

- 初期対応形式、実デコード検証、1秒未満拒否を定義
- 通常枠 5 MB / 3分 / 3件、将来買い切り枠 A・B の上限を定義
...
Play Store 初回提出後の次候補として BACKLOG Phase 13 へ登録した。

---

## Issue #86 Phase A — 通知音からアプリ音源への引き継ぎ (2026-07-15)

PR #112 (commit `651f798`) で、OS 通知音と audioplayers の本音源が重なる問題を解消した。

- 通知 Channel を短い自己終了音へ変更し、Channel ID を更新
- 通知音再生中にアプリ音源を prepare し、固定 3200 ms 後に再生
- Android audio usage を alarm に統一
- 世代トークンで Stop / Snooze / 別タイマー開始後の遅延再生を無効化
- release resource shrink から短音源を保持
- Drift v5 巻き戻し後も v6 migration を再実行できる既存列判定を追加

`flutter analyze`、全テスト (690 passed / 1 skipped)、release APK build に成功。
Pixel 6a / Android 16 で前面、ロック画面、cold launch を確認し、いずれも二重音なし。
サイレントモード中もアラーム音量経路で鳴動することを確認した。

---

## 定間隔通知 (2026-07-13)

PR #111 (commit `aa3872a`) で、タイマー満了後も指定間隔で短い通知音を鳴らす機能を追加した。

- Domain は周期境界でも `running` を維持し、基準時刻ベースでドリフトを防止
- Android Native は自己再予約 Receiver、再起動復元、画面操作からの取消を実装
...
- en スクリーンショットは後続の英語版 Store listing 素材準備で追加撮影済み。
- 画像 / docs only 変更のため `flutter analyze` / `flutter test` は未実行。

---

## Phase 11.9 γ — 英語版 Store listing 素材準備 (2026-06-17)

英語版 Store listing も初回提出範囲に含める方針へ更新した。
既存の `docs/play-store-listing.md` には Short Description / Full Description /
What's new の English 草稿が揃っているため、追加で言語依存のある画像素材を整理した。

### 対応

- `design/store/feature-graphic-1024x500.png` は日本語コピー入りのため ja 用として維持。
- 英語版 Feature Graphic として
  `design/store/feature-graphic-1024x500-en.png` を追加。
- `tasklist.md` / `BACKLOG.md` /
  `docs/oss-and-play-release-plan.md` / `docs/play-store-listing.md` を、
  「en スクリーンショット要否確認」ではなく
  「en スクリーンショット 7 シナリオ追加撮影」に更新。

### 追加撮影

- Pixel 6a を英語表示 (`localeTag=en`) に切り替え、ja と同一の 7 シナリオを
  `design/screenshots/phone/en/` に配置。
- 7 ファイルすべて 1080×2400 PNG。
- 撮影後、端末側の `localeTag` は削除し、システム追従へ戻した。

### 撮影ファイル

- `01_timer_multi_running.png`: Timer タブ、複数タイマー表示。
- `02_stopwatch_laps.png`: Stopwatch、ラップ記録あり。
- `03_alarm_list_repeat_once.png`: Alarm List、単発 + 毎日繰り返し。
- `04_world_clock_analog_6_cities.png`: World Clock、アナログ表示、6 都市登録。
- `05_alarm_ringing_screen.png`: Alarm Ringing 画面。
- `06_settings_theme_language_diagnostics.png`: Settings、テーマ / 言語 / 診断ログトグル可視。
- `07_preset_manage.png`: Preset Manage Screen。

### 検証

- `design/screenshots/phone/en/*.png` の 7 ファイルがすべて 1080×2400 であることを確認。
- Feature Graphic は 1024×500 であることを確認。
- 画像 / docs only 変更のため `flutter analyze` / `flutter test` は未実行。

### 残作業

- Play Console 実画面で listing / Data Safety を確定する。
- upload keystore 実体 + `android/key.properties` 作成後、署名付き AAB を検証する。
