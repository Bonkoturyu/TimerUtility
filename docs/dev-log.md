...
- Auto セッションを別途立ち上げ、本 PR で 3 言語 × 172 翻訳キー = 516 翻訳を
  一括投入。本格翻訳に踏み込むため、Auto 指示書
  ([c:\\tmp\\auto-prompt-a3-zh-ko-translation.md]) を事前にユーザ作成し、
  翻訳指針 (用語選定、全角句読点、CLDR plural rule、`docs/translations.md`
  の 5 列ミラー不採用方針 a など) を凍結してから着手
...
- **partial sync of `homeOpen*` / `*EmptyHint`**: PR #45 (Phase 11) で
  ...
  Stopwatch` 等) が残存。本 PR では自分が追加した「ja / en 列は維持する」
...
2. **zh_Hant が Simplified にフォールバックしない (round 1 bug fix の
   ...
   いた可能性がある (本 PR では production で再現確認はしていないが、
...
642 tests pass / 1 skipped / `flutter analyze` clean / `flutter build apk
--debug --dart-define=ENABLE_EXPERIMENTAL_LOCALES=true` success。

### 持ち越しタスク

- **`docs/translations.md` 一括同期**: 本 PR では `homeOpen*` /
...
- **アプリアイコン・スプラッシュ / Play Store 提出準備**: Phase 11 残
  タスク (`BACKLOG.md` Phase 11 残タスク参照)。本 PR スコープ外
