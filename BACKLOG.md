...
- [ ] HomeScreen を PageView 化（Stopwatch / Timer / Alarm / Clock の 4 機能を左右 swipe で切替、デフォルトページ = Timer。Phase 10.5 実機検証 2026-05-10 のフィードバックで起票）。
      BottomNavigationBar / NavigationBar 併設の是非、kill → 復元時のデフォルトページ復帰、現行 4 ボタン縦並び UI との互換 (一部ユーザは縦並びを好む可能性) は設計時に検討する。
...
| 10.5 | ほぼ完了（PR #27 SafeArea 実機最終確認待ち） | 世界時計。Domain / Infrastructure / Application / Presentation 全層実装 + main.dart 配線 + l10n + docs 更新（Session 1〜5、PR #18-25）。実機検証フィードバック対応 (PageView 循環 / 都市 A-Z / ドット pill 化) は PR #26 でマージ・再検証済 (2026-05-10、Pixel 6a / Android 16、6 シナリオ + 上限ガード OK)。残: PR #27 で nav バー回避 (SafeArea) を入れたので、merge 後に「ドットがシステムボタンと十分離れて描画される」だけ実機確認すれば完全クローズ。504 テストパス |
| 11 | 未着手 | 任意 |
| 12 | 未着手 | 任意 / iOS 版（Android 版完成後） |

---

最終更新日: 2026-05-10（Phase 10.5 ほぼ完了 — PR #27 SafeArea の実機最終確認のみ残。Session 1〜5 で Domain / Infrastructure / Application / Presentation 全層 + 配線 + l10n + docs を実装、PR #26 で実機検証フィードバック (PageView 循環 / 都市 A-Z / ドット pill 化) を反映、Pixel 6a / Android 16 で 6 シナリオ + 上限ガード OK。PR #27 は nav バー回避の SafeArea 追加で merge 後に実機確認すれば完全クローズ。504 テストパス）
