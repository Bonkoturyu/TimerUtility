...
- [ ] アプリアイコン・スプラッシュ (Phase 11.9 で実施予定、
  [docs/oss-and-play-release-plan.md](docs/oss-and-play-release-plan.md))
...
| 11 (その他) | 進行中 | 設定画面 4 項目 (PR #36) + CVD 冗長表示 (PR #39) + 言語手動切替 UI (2026-05-14) + 通知 channel 名 i18n (PR #59、2026-05-16) + F-7 Manifest 整形 (PR #59) + A-3 中韓 ARB 本格翻訳 (zh / zh_Hant / ko、2026-05-16) 完了。残: アプリアイコン / Play Store 提出準備 (Phase 11.8 / 11.9 / 11.10 に分割、計画書 [docs/oss-and-play-release-plan.md](docs/oss-and-play-release-plan.md) PR #66 承認済) |
...
| 11.9 (Play 提出準備) | α・#74 fix・β 素材非依存準備マージ済、β 素材投入待ち | サブ PR α (`phase-11.9-alpha`、**PR #72 main マージ済 2026-05-28**): T0 applicationId rename + I.1 MethodChannel rename + alarm_ringing_screen.dart ハードコード解消 + live docs 5 ファイル追従、Pixel 6a 8 シナリオ OK。Issue #74 fix (Lock 画面 FSI 二重音、`fix/issue-74-fsi-cold-launch-double-sound`、**PR #75 main マージ済 2026-05-29 squash `dcac842`**、issue #74 クローズ): 判定軸を KeyguardManager「Lock 画面表示中か」に補正、unlock 500ms / Lock 1800ms 分岐、Pixel 6a 4 シナリオ OK (シナリオ 4 ×3 連続単音化)、651 passed (1 skipped)。サブ PR β 事前準備 (`phase-11.9-beta-prep`、**PR #77 main マージ済 2026-05-29 squash `0a0509d`**): 素材非依存タスクを front-load — strings.xml 5 言語 + Manifest `android:label` → `@string/app_name` 配線 (T4 相当、OS 表示名 `TimerUtility` 化) + §G アイコン仕様公式裏取り確定 (66dp safe zone / Play 角丸 30%+shadow / `<monochrome>` / sRGB) + §A 版数再確認 (launcher_icons 0.14.4 / native_splash 2.4.7)、Gemini の Play 角丸「20%」指摘は公式逐語「30%」で却下。残既知問題: 新規 install 直後 POST_NOTIFICATIONS 初回ダイアログ + Banner 不表示 (将来 follow-up、tasklist.md 記載)。**次: サブ PR β 本体** (ゴール「素材投入で完了」: アイコン素材 3 層を `design/icon/` に配置 + flutter_launcher_icons / flutter_native_splash 導入 [pubspec.yaml 要確認] + 生成 + Pixel 6a 4 パターン確認) → その後 γ (privacy-policy GitHub Pages + listing + signing + aab) |
...
最終更新日: 2026-05-29（β 素材非依存準備の PR #77 マージ後同期 — branch `docs/sync-plan-after-77`。Phase 11.9 サブ PR β の素材非依存タスク (strings.xml 5 言語 + Manifest `android:label` → `@string/app_name` 配線 [T4 相当] + §G アイコン仕様公式裏取り確定 + §A 版数再確認) を PR #77 (squash `0a0509d`) で main マージ。進捗サマリ表 Phase 11.9 行の状態を「α・#74 fix・β 素材非依存準備マージ済、β 素材投入待ち」に更新し #77 内容を追記。ゴール方針「素材を入れたら β がほぼ完了する状態」をユーザ合意として明記。Gemini code-assist の Play 角丸「20%」指摘は公式ページ逐語「Radius will be equivalent to 30% of icon size.」(2 箇所明記) で却下済。doc-only、`flutter analyze` / `flutter test` 不要）
