...
| 11.9 (Play 提出準備) | サブ PR α main マージ済、次 β | サブ PR α (`phase-11.9-alpha`、**PR #72 main マージ済 2026-05-28**): T0 applicationId rename + I.1 MethodChannel rename + alarm_ringing_screen.dart ハードコード解消 + live docs 5 ファイル追従、Pixel 6a 8 シナリオ OK。Issue #74 fix (Lock 画面 FSI 二重音、`fix/issue-74-fsi-cold-launch-double-sound`、**PR #75 main マージ済 2026-05-29 squash `dcac842`**、issue #74 クローズ): 判定軸を KeyguardManager「Lock 画面表示中か」に補正、unlock 500ms / Lock 1800ms 分岐、Pixel 6a 4 シナリオ OK (シナリオ 4 ×3 連続単音化)、651 passed (1 skipped)。残既知問題: 新規 install 直後 POST_NOTIFICATIONS 初回ダイアログ + Banner 不表示 (将来 follow-up、tasklist.md 記載)。**次: サブ PR β** (アイコン素材 + flutter_launcher_icons + flutter_native_splash + strings.xml 5 言語 + Pixel 6a 4 パターン確認) → その後 γ (privacy-policy GitHub Pages + listing + signing + aab) |
