...
最終更新日: 2026-05-11（Phase 11 ダークモード対応 — `MaterialApp.darkTheme` を deepPurple seed + `Brightness.dark` で追加、`ThemeMode.system` 固定で OS 追従。permission_banners と analog_clock_widget のハードコード色を MD3 semantic role (`errorContainer` / `tertiaryContainer` / `secondaryContainer` / `error` + 対応 `onXxxContainer`) に置換、両モードでコントラスト担保。手動切替 UI / `UserPreferences.themeMode` キー / Riverpod ThemeMode Notifier は設定画面タスクに先送り。527 テストパス (1 skipped)、実機検証は別タスク）
