...
- [ ] 初回「世界時計を開く」tap → 位置情報許可 → 「現在地」時計が自動追加される (権限ダイアログは ClockScreen 入った瞬間に出る、HomeScreen / 起動時には出さない)
- [ ] 同上で位置情報拒否 → 端末タイムゾーン（例: Asia/Tokyo）で「現在地」が登録される
...
| 10.5 | 完了（実機検証待ち） | 世界時計。Domain / Infrastructure / Application / Presentation 全層実装 + main.dart 配線 + l10n + docs 更新済（2026-05-10、502 テストパス、Session 1〜5）。実機検証 (Pixel 6a / Android 16、6 シナリオ) はユーザ手動 |
| 11 | 未着手 | 任意 |
| 12 | 未着手 | 任意 / iOS 版（Android 版完成後） |

---

最終更新日: 2026-05-10（Phase 10.5 完了 — 実機検証待ち。Session 1〜5 で Domain / Infrastructure / Application / Presentation 全層 + main.dart 配線 + l10n + docs 更新を実装、502 テストパス。実機 6 シナリオはユーザ手動）
