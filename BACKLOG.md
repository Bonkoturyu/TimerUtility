...
最終更新日: 2026-05-12（PR #34 で設定画面タスクに「テーマ手動切替」と「色覚多様性 (CVD、いわゆる色盲) 対応モード」を併記、レビュー指摘を反映してテーマ永続化を `UserPreferences` の `int` (`ThemeMode.index`) ベースに修正 + Material 3 出典記述を技術的事実に書き換え。Phase 11 ダークモード対応本体は 2026-05-11 (PR #33) で `MaterialApp.darkTheme` を deepPurple seed + `Brightness.dark` で追加、`ThemeMode.system` 固定で OS 追従。permission_banners と analog_clock_widget のハードコード色を MD3 semantic role (`errorContainer` / `tertiaryContainer` / `secondaryContainer` / `error` + 対応 `onXxxContainer`) に置換、両モードでコントラスト担保。手動切替 UI / Riverpod ThemeMode Notifier は設定画面タスクに先送り。527 テストパス (1 skipped)、実機検証は別タスク）
