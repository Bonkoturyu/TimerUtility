...
- [ ] 実機検証: 複数タイマー同時稼働 + アプリ再起動後の状態復元 + 過去到達タイマーの completed 通知（Auto 範囲外）
...
- [ ] `domain/alarm/day_of_week.dart`（enum、`DateTime.weekday` と互換マッピング）
- [ ] `domain/alarm/alarm_repeat.dart`（sealed class: `AlarmRepeatOnce` / `AlarmRepeatWeekly(Set<DayOfWeek>)`）+ Unit Test
- [ ] `domain/alarm/alarm_entity.dart`（freezed、`docs/domain-model.md` の AlarmEntity 定義に従う）
- [ ] `domain/alarm/alarm_service.dart` 実装 + Unit Test
  - `nextFireAt` / `advanceAfterFire` / `snoozeUntil` の 3 メソッド
  - `Clock` 注入、副作用なし
  - 曜日跨ぎ、日付跨ぎ、夏時間相当のエッジケースをカバー
- [ ] `domain/alarm/exceptions.dart`（`AlarmNotFoundException` / `MaxAlarmCountExceededException` / `InvalidAlarmRepeatException` / `InvalidSnoozeMinutesException`）
- [ ] `domain/ports/alarm_repository.dart`（`add` / `update` / `delete` / `findById` / `findAll`）

### Infrastructure 層

- [ ] `infrastructure/database/app_database.dart` に `alarms` テーブル追加（Drift スキーマ migration）
- [ ] `infrastructure/database/drift_alarm_repository.dart` 実装 + Unit Test（in-memory DB）
- [ ] `infrastructure/database/mappers/alarm_mapper.dart`（AlarmEntity ⇔ AlarmsCompanion）
  - `repeat: AlarmRepeat` の永続化形式は JSON or 専用列（実装時に決定）

### Application 層

- [ ] `application/alarm_repository_provider.dart`
- [ ] `application/alarm_service_provider.dart`
- [ ] `application/alarm_collection_notifier.dart` 実装 + Unit Test
  - State: `List<AlarmEntity>`
  - `load` / `create` / `update` / `toggle(AlarmId)` / `delete(AlarmId)`
...
- [ ] `application/alarm_ringing_notifier.dart` を Timer / Alarm 両用化
  - payload prefix `timer:` / `alarm:` で起動元判別
  - 既存テストは regression（Timer 由来パスは挙動不変）
- [ ] `lib/main.dart` の `onDidReceiveNotificationResponse` を payload prefix 対応に拡張

### Presentation 層

- [ ] `presentation/screens/alarm_list_screen.dart` 実装 + Widget Test
  - アラーム一覧（時刻 / 曜日 / ラベル / ON-OFF トグル）
  - 各行タップで edit 画面へ
  - FAB で新規作成
- [ ] `presentation/screens/alarm_edit_screen.dart` 実装 + Widget Test
  - TimePicker / 曜日チップ / ラベル / 音源選択 / スヌーズ分（5/10/15）
  - 削除ボタン（既存アラーム時のみ）
- [ ] `presentation/widgets/weekday_selector.dart`（曜日チップ multi-select）+ Widget Test
- [ ] go_router に `/alarms` / `/alarms/edit/:id?` ルート追加
- [ ] HomeScreen に Stopwatch / Timer / Alarm の 3 本柱導線を追加（or BottomNavigationBar 化）

### docs 更新

- [ ] `docs/architecture.md` のディレクトリ構造図に `lib/domain/alarm/` を追記
- [ ] `docs/domain-model.md` は Phase 9.5 設計時点の定義のため、実装で乖離が出たら追記

### 実機検証

- [ ] Pixel 6a / Android 16 で以下の 4 シナリオを確認:
...
- [ ] **AlarmCollectionNotifier 経由で全 enabled アラームを再予約**（Phase 9.5 と連携）
- [ ] 実機での再起動テスト（Timer / Alarm 両方の復元を確認）
...
- 再起動中に時刻を過ぎたアラームは AlarmService の once / weekly ルールに従って処理される

**依存**: Phase 9.5 完了
**参照**: `docs/android-constraints.md`, `docs/platform-channels.md`, `docs/adr/0005-alarm-vs-timer-separation.md`
...
| 8 | 完了（実機検証待ち） | 複数タイマー (上限 10) + Drift 永続化 + 起動時復元 (過去到達は completed + show 通知)、180 テストパス（2026-05-01）、docs 反映待ち |
| 9 | 未着手 | |
...
最終更新日: 2026-05-01（Phase 8 完了反映: 複数タイマー + Drift 永続化、180 テストパス）
