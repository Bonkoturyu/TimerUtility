...
最終更新日: 2026-05-12（Phase 11 ダークモード対応の実機検証完了 + プリセット管理の発見性改善 + 設定画面タスクへの併記 3 件 — ダークモード本体は 2026-05-11 (PR #33) で `MaterialApp.darkTheme` を deepPurple seed + `Brightness.dark` で追加、`ThemeMode.system` 固定で OS 追従。permission_banners と analog_clock_widget のハードコード色を MD3 semantic role (`errorContainer` / `tertiaryContainer` / `secondaryContainer` / `error` + 対応 `onXxxContainer`) に置換、両モードでコントラスト担保。Pixel 6a / Android 16 で 7 シナリオ全 OK (S2 (e) preset 削除確認の skip と S7 Recents スナップショットは仕様で対応不要)。実機検証フィードバックを受けて `PresetSelectSheet` 末尾に「プリセットを管理...」エントリを追加し、Timer タブ overflow menu に隠れていたプリセット管理導線を sheet 内からも辿れるよう改善 (PR #35)。あわせて PR #34 で設定画面タスクに「テーマ手動切替」と「色覚多様性 (CVD、いわゆる色盲) 対応モード」を併記済 (レビュー指摘を反映してテーマ永続化を `UserPreferences` の `int` (`ThemeMode.index`) ベースに修正 + Material 3 出典記述を技術的事実に書き換え)。手動切替 UI / Riverpod ThemeMode Notifier / CVD-safe palette 切替は設定画面タスクに先送り。528 テストパス (1 skipped)）
