...
- [x] 実機検証: Pixel 6a / Android 16 で 4 シナリオ確認 (2026-05-09 1 巡目で manifest 不具合発覚)
  1. [x] 5 分後 timer 設定 → 端末再起動 → 4 分後に通知発火 (OK)
  2. [ ] 翌朝の once-mode alarm 設定 → 端末再起動 → 翌朝定刻に発火
        (1 巡目で **NG**: 再起動後に AlarmManager 再登録されず past-due
        検知でアプリ起動時に救済される挙動を観測。原因は
        `ScheduledNotificationBootReceiver` が `exported="false"` で
        Android 12+ の OS broadcast 配信要件を満たしていなかった
        ことを確定。`exported="true"` に修正済 — 要再検証)
  3. [ ] 過去到達 once-mode alarm を擬似的に作成 → 起動後 enabled=false +
        show 通知 1 回 (1 巡目では端末日時操作で AlarmManager 予約が
        natural fire し past-due 検知を経由しなかった。再検証手順:
        force-stop → 端末日時を target 後ろまで進める → launcher 起動)
  4. [x] weekly alarm (明日の曜日のみ) を再起動跨ぎで翌日定刻に発火 (OK)
...
| 10 | 実装完了 (再検証待ち) | Timer + Alarm の起動時復元 + past-due once-mode 取り下げ。1 巡目実機 (2026-05-09) でシナリオ 1/4 OK、シナリオ 2 NG → `ScheduledNotificationBootReceiver` の `exported="true"` 修正で根本対応、シナリオ 2/3 の再検証残 |
...
最終更新日: 2026-05-09（Phase 10 1 巡目実機検証で `ScheduledNotificationBootReceiver` の Android 12+ exported 要件不備が発覚、manifest を `exported="true"` に修正。シナリオ 2/3 の再検証残）
