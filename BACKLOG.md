...
- [ ] 実機検証: Pixel 6a / Android 16 で 4 シナリオ確認
  1. 5 分後 timer 設定 → 端末再起動 → 4 分後に通知発火
  2. 翌朝の once-mode alarm 設定 → 端末再起動 → 翌朝定刻に発火
  3. 過去到達 once-mode alarm を擬似的に作成 → 起動後 enabled=false +
     show 通知 1 回
  4. weekly alarm (明日の曜日のみ) を再起動跨ぎで翌日定刻に発火
...
| 10 | 実装完了 (実機検証待ち) | Timer + Alarm の起動時復元 + 過去到達 once-mode の取り下げ実装済、Pixel 6a で 4 シナリオ実機検証残 |
...
最終更新日: 2026-05-04（Phase 10 実装完了、AlarmCollectionNotifier に過去到達 once-mode 検知 + 取り下げを追加、実機検証 4 シナリオ待ち）
