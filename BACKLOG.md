...
- [x] Native → Flutter のイベント送信仕様確定（`docs/platform-channels.md` 更新）（2026-05-13、Phase 9.5 / 10 / 10.5 / 11 で能動イベント不要と確定したため docs を実装ベースに整列して closing）
...
- [x] `domain/timer/snooze_calculator.dart` 実装 + Unit Test
- [x] `TimerService` にスヌーズメソッド追加 + Unit Test
- [x] `NotificationScheduler` のキャンセル + 再予約フロー
- [x] AlarmRingingScreen にスヌーズボタン動作実装
- [x] スヌーズ時間プリセット（3 分 / 5 分 / 10 分）UI（`SnoozeCalculator.allowedMinutes` で実装）
- [x] `presentation/widgets/duration_picker.dart` 新規実装 + Widget Test（時 / 分 / 秒のホイール選択、上限 99 時間）
- [x] TimerScreen のプリセットチップに「カスタム」エントリ追加 → DurationPicker をモーダル表示 → `TimerNotifier.create` 呼び出し + Widget Test（Phase 9 で `PresetSelectSheet` に統合）
...
- [x] `infrastructure/database/app_database.dart` に `clock_locations` テーブル追加（Drift schema migration、schemaVersion 3 → 4）
- [x] `infrastructure/database/drift_clock_location_repository.dart` 実装 + Unit Test（in-memory DB、findAll は displayOrder ASC、replaceAll は transaction で delete → batch insert）
- [x] `infrastructure/database/mappers/clock_location_mapper.dart`（ClockLocation ⇔ ClockLocationsCompanion / Row）
- [x] `infrastructure/location/location_detector_adapter.dart`
  - `geolocator` で coarse location 取得 → `geocoding` で逆ジオコーディング → 国コード → `CountryToTimezone` で代表 TZ 解決
  - 失敗時 (権限拒否 / オフライン / 逆ジオコ失敗) は `FlutterTimezone.getLocalTimezone()` → 最終 fallback `Asia/Tokyo`
  - 補助マップ: `infrastructure/location/country_to_timezone.dart` (約 40 ヶ国、`TimezoneCatalog` の単一 TZ 国を全カバー)
  - Unit Test は country_to_timezone のみ。adapter 自体は MethodChannel 依存で mock 困難なため実機検証で担保
- [x] `infrastructure/clock/timezone_catalog.dart`
  - 主要都市プリセット 25 件のみ実装（Tokyo / Seoul / Shanghai / HK / Singapore / Bangkok / Kolkata / Dubai / Moscow / Berlin / Paris / London / Sao Paulo / NY / Toronto / Mexico City / Chicago / Denver / LA / Vancouver / Anchorage / Honolulu / Sydney / Auckland）
  - **国コード → 代表 TZ マップは本セッションでは未実装**。`location_detector_adapter` と一緒に次セッションで追加する

### Application 層

- [x] `application/clock_location_repository_provider.dart` (preset 流儀 stub、main.dart で override)
- [x] `application/location_detector_provider.dart` (同上)
- [x] `application/clock_collection_notifier.dart` 実装 + Unit Test
  - State: `ClockCollection` (集約直接、preset 流儀)
  - `addPreset(timezoneId, displayName)` / `remove(id)` / `reorder(oldIndex, newIndex)` / `update(id, displayName?)`
  - 起動時 DB 復元 (`build()` 内 microtask)
  - 初回起動時（DB 空）に `LocationDetector.detectTimezoneId()` → 「現在地」として登録
  - 上限超過時は aggregate が `MaxClockLocationCountExceededException` を throw
  - main.dart で `DriftClockLocationRepository` / `LocationDetectorAdapter` を override 注入済
- [x] `application/clock_tick/current_time_stream_provider.dart` 実装（既存 `docs/state-management.md` で定義済み、ここで実装）
  - `Stream.multi` で初回即時 emit + 1 秒周期 `Timer.periodic` + `Clock` 経由で `DateTime` を流す
  - autoDispose、画面表示中のみ active

### Presentation 層

- [x] `presentation/widgets/analog_clock_widget.dart` + Widget Test
  - `CustomPainter` で時針 / 分針 / 秒針描画、引数で `DateTime` と `Size` を受け取る
- [x] `presentation/widgets/digital_clock_widget.dart` + Widget Test
  - `HH:mm:ss` + 都市名 + UTC オフセット表示
- [x] `presentation/widgets/clock_design_a.dart` / `clock_design_b.dart` / `clock_design_c.dart`
  - 3 種類のデザインバリエーション。Grid 内で `ClockLocation` × `DateTime` を描画
- [x] `presentation/screens/clock_screen.dart` + Widget Test
  - `PageView` でデザイン A/B/C 切替（横スワイプ + ドットインジケーター）
  - 各ページに最大 6 個の時計を 2x3 Grid 表示
  - FAB / Edit ボタンで `/clock/locations` へ
- [x] `presentation/screens/clock_location_picker_screen.dart` + Widget Test
  - プリセット都市から追加選択（上限 6 個に達したら disable）
  - 並べ替え（`ReorderableListView`）+ 削除
- [x] `lib/main.dart` の `go_router` に `/clock` / `/clock/locations` ルート追加
- [x] `HomeScreen` に「Open Clock」ボタンを追加（Stopwatch / Timer / Clock の 3 本柱、または BottomNavigationBar 化検討）

### Manifest / pubspec（要ユーザー確認）

- [x] `pubspec.yaml` に `geolocator: ^14.0.2` / `geocoding: ^4.0.0` 追加（Phase 10.5 実装時に反映済）
- [x] `android/app/src/main/AndroidManifest.xml` に `ACCESS_COARSE_LOCATION` 追加（Phase 10.5 実装時に反映済）
- [x] `docs/permissions.md` に位置情報権限フローを追記（取得タイミング: 初回 ClockScreen 進入時のみ、拒否時は FlutterTimezone fallback。docs/permissions.md L40, L43 既述）

### docs 更新

- [x] `docs/architecture.md` のディレクトリ構造図に `lib/domain/clock/`, `infrastructure/location/`, `infrastructure/clock/` を追記
- [x] `docs/domain-model.md` に Clock Aggregate（ClockLocation / ClockCollection / ClockTime）を追加
- [x] `docs/state-management.md` に `clockCollectionNotifierProvider` 等を追加、`currentTimeStreamProvider` を実装済みに更新

### 実機検証（Pixel 6a / Android 16）

- [x] 初回「世界時計を開く」tap → 位置情報許可 → 「現在地」時計が自動追加される (Tokyo として登録、2026-05-10)
- [x] 同上で位置情報拒否 → 端末タイムゾーン（例: Asia/Tokyo）で「現在地」が登録される (2026-05-10)
- [x] 地域追加（LA / NY / London 等）→ Grid に表示、秒単位で更新 (2026-05-10)
- [x] PageView スワイプでデザイン A/B/C 切替 (PR #26 で C↔A 循環追加、2026-05-10 再検証で OK)
- [x] アプリ強制終了 → 再起動 → 時計リストが復元される（Drift） (2026-05-10)
- [x] 並べ替え / 削除動作 (2026-05-10)
- [x] 上限ガード (6 件 → 7 件目で SnackBar) (2026-05-10)
- [x] (PR #26 で対応) ドットインジケータ可視性、都市カタログ A-Z 並び。再検証時に 3-button ナビゲーション
      バーとの重なりが指摘され、PR #27 で SafeArea 対応 (2026-05-10)

**DoD**:

- 1 画面で最大 6 都市の現在時刻が秒単位で更新される
- 初回 ClockScreen 進入時に GPS で現在地を 1 度だけ取得、以降は端末 TZ (HomeScreen / アプリ起動時には権限要求しない)
...
- [~] 設定画面（音源選択、デフォルトスヌーズ時間、テーマ手動切替、色覚多様性対応モードなど）
  ...
  - CVD モード: 2026-05-13、PR #XX で完了 (方針 (a) 冗長表示、3 種バナーすべてに
    重大度ラベル併記 + FontWeight 段階差 + 左端色帯の形状差)。
  - **テーマ手動切替**: `MaterialApp.themeMode` を `UserPreferences` に
    ...
    `UserPreferenceKeys.themeMode` を新規追加 (現状の `UserPreferences` ポートは
  ...
  - [x] **色覚多様性 (CVD、いわゆる色盲) 対応モード** (2026-05-13、PR #XX 完了、方針 (a) 採用): 現状 `permission_banners.dart` の
    ...
    で重大度を「色のみ」で区別している。1 型 / 2 型色覚で色相差が縮んだ際にも
    重大度が読み取れるよう、以下のいずれかを検討:
    - (a) **冗長表示で対応** (推奨デフォルト): バナーのアイコン + タイトル先頭に
      重大度ラベル (例: `[重要]` / `[推奨]` / `[補助]`) を併記、`FontWeight.bold`
      の使い分けや左端の色帯の幅で形状差も付ける。設定不要で全ユーザに適用。
    - (b) **CVD-safe palette への切替設定**: 設定で「色覚多様性対応モード」
      ON/OFF を持ち、ON 時は Okabe-Ito の CVD-safe 8 色や IBM Color Palette
      等の代替パレットに切り替える。`ColorScheme.fromSeed` の seed を
      切り替える / カスタム `ColorScheme` を渡す の 2 通り。
    - 上記 (a) / (b) どちらを取るか、または両方併用かは設定画面着手時に再検討。
      Material 3 の seed-based palette は WCAG AA contrast を満たす設計だが、
      色相のみで区別している役割 (例: error vs tertiary) は CVD タイプによって
      識別困難になる。これは MD3 の保証範囲外なので別途 CVD タイプ別の検証で
      担保する必要がある。
    - 検証: 検証時は実機 + Android の「色補正」(Settings → Accessibility →
      Color correction) を使って各 CVD タイプ (Protanomaly / Deuteranomaly /
      Tritanomaly) で見え方を確認する。CI に組み込む CVD シミュレーションは
      別途検討 (現状 Flutter のテストハーネスに CVD フィルタは無い)。
- [x] HomeScreen を PageView 化（2026-05-10、Phase 10.5 実機検証フィードバックで起票）。
      Stopwatch / Timer / Alarm / Clock を左右 swipe で切替、デフォルトページ = Timer (index 1)。
      kill→再起動復元は `UserPreferences.lastHomePageIndex` で永続化。BottomNavigationBar /
      NavigationBar は採用せず PageView + AppBar `PageNavigationHint` + `HomeDotIndicator` の構成。
      Clock タブは内部 PageView を SegmentedButton (Analog / Digital / Compact) に置換し外側
      タブ swipe との gesture 競合を根本除去。各タブの body は
      `lib/presentation/screens/home/<feature>_page.dart` に切り出し、deep link Screen は薄ラッパ
...
- [x] ClockLocationPicker 周辺の内部識別子リネーム（2026-05-11 起票、2026-05-11 完了。PR #29 で
      ...
      `clockLocationPickerSectionPinned` 等が `Location` (都市) 由来のまま残っている。
      本機能の本質は「複数時計の追加・編集 (= 時差確認 UI)」なので識別子も
      `ClockEntryEditScreen` / `/clock/entries` 等に揃えるのが整合的だが、
      影響範囲 (テスト / docs / コメント) が広く UX 改善 PR とは別 PR に分離。
      未参照になった `clockMenuEditLocations` ARB エントリ削除もこのリネーム
      PR にまとめてクリーンアップする。
- [x] Clock ドメイン層の「Location」系識別子リネーム（2026-05-11 起票、2026-05-11 完了。
      案 A 全面リネーム採用: `ClockLocation` → `ClockEntry`、
      `ClockCollection` → `ClockEntryCollection`、Drift テーブル `clock_locations`
      → `clock_entries` (schemaVersion 4 → 5 で `INSERT...SELECT` migration)、
...
- [x] ダークモード対応（2026-05-11、`MaterialApp.darkTheme` を deepPurple seed +
      `Brightness.dark` で追加。`themeMode` は省略 → `ThemeMode.system` で
      OS 設定に追従。ハードコード色 (permission_banners の 3 種背景、
      analog_clock_widget の秒針) を MD3 semantic role
      (`errorContainer` / `tertiaryContainer` / `secondaryContainer` /
      `error` + 対応 `onXxxContainer`) に置換し両モードで読みやすさを担保。
      手動切替 UI / `UserPreferences.themeMode` キーは設定画面タスクに
      先送り。Pixel 6a / Android 16 で 7 シナリオ実機検証完了 (2026-05-12)、
      S2 (e) preset 削除確認ダイアログは `skipPresetDeleteConfirm` 永続化が
      効いた仕様、S7 Recent 画面のテーマ非追従は Android スナップショット仕様で
      いずれも対応不要、他 6 シナリオ全 OK）
- [x] プリセット管理の発見性改善（2026-05-12、Phase 11 ダークモード実機検証で起票、
      同日に実機検証完了、PR #35）。
      Timer タブ AppBar overflow にしか「プリセット管理」導線がなく発見性が低い
      問題に対し、`PresetSelectSheet` 末尾 (custom button の下、Divider 区切り)
      に `TextButton.icon` の「プリセットを管理...」エントリを追加。
      `PresetSelectResult` に `manageRequested` フィールドを追加して caller
      (`timer_list_page._onAddPressed`) で `context.push('/presets')` 分岐。
      Overflow menu の旧導線は残置 (両方からアクセス可)。
      Pixel 6a / Android 16 で 6 シナリオ実機検証完了 (sheet 末尾エントリ表示 /
      Manage button → `/presets` 遷移 / empty 状態でも表示 / overflow 旧導線
      リグレッションなし / ライト・ダーク両モードで視認性 OK)、不具合なし
...
| 10.5 | 完了（実機検証済み） | 世界時計。Domain / Infrastructure / Application / Presentation 全層実装 + main.dart 配線 + l10n + docs 更新（Session 1〜5、PR #18-25）。実機検証フィードバック対応 (PageView 循環 / 都市 A-Z / ドット pill 化) は PR #26 で、システム nav バー回避 (SafeArea bottom) は PR #27 でそれぞれマージ・実機確認済 (2026-05-10、Pixel 6a / Android 16、6 シナリオ + 上限ガード + ドットの nav バー余白すべて OK)。504 テストパス |
| 11 (HomeScreen PageView) | 完了（実機検証待ち） | HomeScreen を 4 タブ横 PageView 化。各タブの body を `lib/presentation/screens/home/<feature>_page.dart` に Page widget として切り出し、deep link Screen は薄ラッパ化。`UserPreferences` port を `getInt` / `setInt` 拡張して `lastHomePageIndex` で kill→復元対応。Clock タブは SegmentedButton 化で内外 PageView の gesture 競合を回避。HomeDotIndicator + PageNavigationHint で視覚ガイド。518 テストパス。実機検証は別タスク (2026-05-10) |
| 11 (ダークモード) | 完了（実機検証待ち） | `MaterialApp.darkTheme` を deepPurple seed + `Brightness.dark` で追加、`ThemeMode.system` 固定で OS 追従。permission_banners と analog_clock_widget のハードコード色を MD3 semantic role に置換。527 テストパス (1 skipped)。手動切替 UI / 永続化は設定画面タスクに先送り。実機検証は別タスク (2026-05-11) |
| 11 (その他) | 進行中 | 設定画面 4 項目 (テーマ / スヌーズ / アラーム音源 / ライセンス導線、PR #36) + CVD 冗長表示 (PR #XX、2026-05-13) 完了。残: アプリアイコン / 言語手動切替 UI / 通知 channel 名 i18n / 中韓 ARB 本格翻訳 / Play Store 提出準備 |
| 12 | 未着手 | 任意 / iOS 版（Android 版完成後） |

---

最終更新日: 2026-05-13（Phase 11 CVD banner labels 完了 — 設定画面サブタスク「色覚多様性 (CVD) 対応モード」を方針 (a) 冗長表示で全ユーザ適用として完結。`permission_banners.dart` の 3 種バナー (post_notifications / exact_alarm / full_screen_intent) に重大度ラベル `[重要]` / `[推奨]` / `[補助]` 併記 + `FontWeight` 段階差 (w900/w700/w600) + 左端色帯 (8/5/3 pt) で形状差を追加。MD3 semantic role による色相区別は維持。ARB 日英に `permissionBannerSeverity{Critical,Recommended,Supplementary}` キー追加、中韓は本 PR 対象外。558 テストパス (1 skipped)、PR #XX）

過去の更新: 2026-05-13（Phase 6 docs cleanup 完了 — `docs/platform-channels.md` を実装ベースに全面リライト、4 ch (`/notification` / `/alarm_event` / `/boot` / `/lockscreen`) の採用見送りを正式に記録、`clearShowWhenLocked` メソッド (Phase 6 実機検証フォローで後付け追加、docs 未反映だった) を文書化、BACKLOG L180 残課題を [x] 化、Phase 6 行を「完全クローズ」表記に更新。コード変更なし）

過去の更新: 2026-05-12（Phase 11 ダークモード対応の実機検証完了 + プリセット管理の発見性改善 (実機検証完了) + 設定画面タスクへの併記 3 件 — ダークモード本体は 2026-05-11 (PR #33) で `MaterialApp.darkTheme` を deepPurple seed + `Brightness.dark` で追加、`ThemeMode.system` 固定で OS 追従。permission_banners と analog_clock_widget のハードコード色を MD3 semantic role (`errorContainer` / `tertiaryContainer` / `secondaryContainer` / `error` + 対応 `onXxxContainer`) に置換、両モードでコントラスト担保。Pixel 6a / Android 16 で 7 シナリオ全 OK (S2 (e) preset 削除確認の skip と S7 Recents スナップショットは仕様で対応不要)。実機検証フィードバックを受けて `PresetSelectSheet` 末尾に「プリセットを管理...」エントリを追加し、Timer タブ overflow menu に隠れていたプリセット管理導線を sheet 内からも辿れるよう改善 (PR #35)、同日に Pixel 6a で 6 シナリオ実機検証完了 (不具合なし)。あわせて PR #34 で設定画面タスクに「テーマ手動切替」と「色覚多様性 (CVD、いわゆる色盲) 対応モード」を併記済 (レビュー指摘を反映してテーマ永続化を `UserPreferences` の `int` (`ThemeMode.index`) ベースに修正 + Material 3 出典記述を技術的事実に書き換え)。手動切替 UI / Riverpod ThemeMode Notifier / CVD-safe palette 切替は設定画面タスクに先送り。528 テストパス (1 skipped)）
