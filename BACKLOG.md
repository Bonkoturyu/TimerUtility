...
| 11.8 (OSS 公開) | 進行中 (T10 待ち) | T1〜T9 PR #67 で main マージ済 (2026-05-16)。T8.5/T8.6 は 2026-05-27 omit 決定 (Privacy team 11 日無反応 + orphan commit `f2e46e3` 残留内容に典型 PII ゼロを実物確認、ユーザー判断でリスク許容)。残: T10 (Public 化) はユーザ作業 (不可逆) |
| 11.9 (Play 提出準備) | 事前検討着手 | branch `phase-11.9-prep` (2026-05-17) — 計画書 [docs/oss-and-play-release-plan.md](docs/oss-and-play-release-plan.md) Phase 11.9 セクションの事前検討 4 件 (A 依存版数 / B applicationId 影響範囲 / C 5 言語アプリ名 / G アイコン仕様) を [docs/phase-11.9-prep-notes.md](docs/phase-11.9-prep-notes.md) に集約 + 実アーティファクト 4 件 (privacy-policy ja/en、play-store-listing、release-signing) を docs/ 配下に草稿作成。`phase-11.9-prep-notes.md` §I のキックオフ判断 4 件はユーザ承認済 (推奨案 A をすべて採用: MethodChannel 名 T0 同 PR 移行 / monochrome layer 常時作成 / アプリ名 5 言語 `TimerUtility` 統一 / α・β・γ 3 PR 分割)。Native / pubspec.yaml は未着手。Phase 11.8 完了 → サブ PR α (T0 + MethodChannel rename + live docs 追従) 着手予定 |
