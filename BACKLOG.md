...
| 11.9 (Play 提出準備) | サブ PR α 実装完了 (Pixel 6a 検証待ち) | branch `phase-11.9-alpha` (2026-05-27、ベース `phase-11.8-close-out`)。T0 applicationId rename (`com.bonkotu.timer.timer_utility` → `io.github.bonkoturyu.timer_utility`) + I.1 MethodChannel rename + alarm_ringing_screen.dart ハードコード解消 + live docs 5 ファイル追従 (README / architecture / android-constraints / permissions / platform-channels)。`flutter analyze` 0 issues / `flutter test` 642 passed / `check_translations_doc.dart` aligned、PR 作成済。AndroidManifest.xml は触らず (事前検討メモ §B.1 で変更不要を確認済)。残: Pixel 6a 実機検証 (Phase 6 FSI + Phase 8.5 アラーム単音化回帰、ユーザ実施) → 検証 OK → main マージはユーザ判断。次の着手単位: サブ PR β (アイコン素材 + flutter_launcher_icons + flutter_native_splash + strings.xml 5 言語 + Pixel 6a 4 パターン確認) |
