...
- [ ] 設定画面（音源選択、デフォルトスヌーズ時間、テーマ手動切替、色覚多様性対応モードなど）
  - **テーマ手動切替**: `MaterialApp.themeMode` を `UserPreferences.themeMode` キー
    (`'light'` / `'dark'` / `'system'`) で永続化、Riverpod `ThemeMode` Notifier 経由で
    MaterialApp に注入。Phase 11 ダークモード対応 (PR #33) では `ThemeMode.system` 固定
    にして手動切替 UI を意図的に持ち越したので、本タスクで蓋を閉じる。
  - **色覚多様性 (CVD、いわゆる色盲) 対応モード**: 現状 `permission_banners.dart` の
    ...
    - 上記 (a) / (b) どちらを取るか、または両方併用かは設定画面着手時に再検討。
      Material 3 公式は seed-based palette が WCAG AA を満たすことを保証している
      が CVD-safe は保証していない (出典: m3.material.io, 2026-05 時点の
      Color system docs)。
...
最終更新日: 2026-05-11（Phase 11 ダークモード対応 + 設定画面タスクに「テーマ手動切替」と「色覚多様性 (CVD、いわゆる色盲) 対応モード」を併記 — ダークモード本体は `MaterialApp.darkTheme` を deepPurple seed + `Brightness.dark` で追加、`ThemeMode.system` 固定で OS 追従。permission_banners と analog_clock_widget のハードコード色を MD3 semantic role (`errorContainer` / `tertiaryContainer` / `secondaryContainer` / `error` + 対応 `onXxxContainer`) に置換、両モードでコントラスト担保。手動切替 UI / `UserPreferences.themeMode` キー / Riverpod ThemeMode Notifier は設定画面タスクに先送り。527 テストパス (1 skipped)、実機検証は別タスク。CVD 対応モードは現状「色のみ」で重大度を区別している権限バナーへの (a) 冗長表示 / (b) CVD-safe palette 切替 の 2 案を設定画面着手時に再検討する前提で起票）
